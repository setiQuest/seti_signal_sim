...
object SETISim {
  ...
  def sparkSim(numPartitions: Int,
               nSim: Int,
               paramGenName: String,
               noiseName: String,
               dataClass: String) {
    ...
    if (noiseName == "sunnoise") {
      ...
      val dashdbForSun: DashDB = new DashDB(
        props.getProperty("JDBC_URL"),
        props.getProperty("DASHDBUSER"),
      ...
      while (sunnoise.next) {
        ...
        if (rand.nextDouble > 0.0) { //keep this fraction secret!  some number of the simulated noise files is gaussian!
          noiseArray(counter) = (
            counter,
            sunnoise.getString("container"),
            sunnoise.getString("objectname"),
            sunnoise.getString("uuid")
          )
        } else {
    ...
    val rdd2 = rdd.mapPartitionsWithIndex { (indx, iter) =>
      ...
      val dashDBConnection: DashDB = new DashDB(
        props.getProperty("JDBC_URL"),
        props.getProperty("DASHDBUSER"),
        props.getProperty("DASHDBPASS"),
        dashdb_database_name
        ...
        .map(i => {
          ...
          if (status == "success") {
            ...
            val DS = new DataSimulator(
              ...
              sigdef.SNR,
              sigdef.drift,
              sigdef.driftRateDerivate,
              sigdef.sigmaSquiggle,
              sigdef.outputLength,
              sigdef.ampModType,
              sigdef.ampModPeriod,
              sigdef.ampModDuty,
            ...
            try {
              ...
              objstore.put(
                simulatedSignalContainer,
                outputFileName,
                dataOutputByteStream.toByteArray
              )
    ...
    println(
      "Returned: " + results.length + " simulations out of " + nSim + " requested of type " + paramGenName
    )
  ...
  def serialSim(nSim: Int,
                paramGenName: String,
                noiseName: String,
                local: Boolean,
                dataClass: String,
                signalAmp: Option[Double]) {
    ...
    for (i <- 0 until nSim) {
      ...
      //8-bit range
      println(
        "Number of X pol amplitude beyond dynamic range " + DS.numBeyondDynamicRangeX
      )
      println(
        f"                ${DS.numBeyondDynamicRangeX} / ${DS.numberOfDataSamples} : ${100 * DS.numBeyondDynamicRangeX / DS.numberOfDataSamples.toFloat}%.2f %%"
      )
      println(
        "Number of Y pol amplitude beyond dynamic range " + DS.numBeyondDynamicRangeY
      )
      println(
        f"                ${DS.numBeyondDynamicRangeY} / ${DS.numberOfDataSamples} : ${100 * DS.numBeyondDynamicRangeY / DS.numberOfDataSamples.toFloat}%.2f %%"
      )
      ...
      try {
        ...
        if (!local) {
          var etag =
            objstore.put(simulatedSignalContainer, outputFileName, dataBytes)
